...
これは、**境界や責務に影響する変更** に対する既定の編集順序である。  
軽微な表記修正、タイポ、実装追従だけの図修正では、この順に厳密に編集しなくてもよい。
...
- 境界、責務、操作対象が変わる変更では、原則としてユースケース図を最初に確認または更新する
- 境界や責務に関わる用語を増やすときは、前提ユースケースが整理済みか確認する
- ドメインモデリングの範囲を曖昧にしたまま、先にモデル図だけを描かない

### 例外

次のような場合は、必ずしもユースケース図ファイルを最初に編集しなくてよい。

- タイポ修正
- 既存図の表記ゆれ修正
- 境界や責務を変えない UML の追従修正
- 用語の意味を変えない日本語 / 英語表記の微修正
...
- まず、集約境界そのものを見直せないか検討する
- それでも別集約のまま扱うなら、更新順序や外部連携の調停はユースケース層に置く
- 強整合で扱う範囲と、最終的整合性で扱う範囲を分けて文書化する
- 必要に応じて、ドメインイベント、outbox、eventual consistency、saga などの現実解を検討してよい
- その場合でも、「何を強整合で守るか」は集約設計の議論として先に明確にする
